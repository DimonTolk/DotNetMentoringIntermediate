...
namespace Expressions.Task3.E3SQueryProvider
{
    public class FtsRequestGenerator
    {
        ...
        public Uri GenerateRequestUrl(Type type, string query = "*", int start = 0, int limit = 10)
        {
            ...
            var nestedQueries = query.Split(Constants.Delimiter);
            var ftsQueryRequest = new FtsQueryRequest
            {
                Start = start,
                Limit = limit
            };
            if (nestedQueries.Length > 1)
            {
                ftsQueryRequest.Statements = new List<Statement>();
                foreach (var nested in nestedQueries)
                {
                    ftsQueryRequest.Statements.Add(new Statement { Query = nested });
                }
            }
            else
            {
